...
async function main() {
	...
	const johan = await prisma.user.upsert({
		...
		create: {
			...
			password: "$2b$10$A./Lo6nel9oEaDLqUJW2QOPSBDE7GkjkGCHixpSsf1fMI62mrOOUi",  // abc123
		},
	});

	const sean = await prisma.user.upsert({
		where: { id: 2 },
		update: {},
		create: {
			id: 2,
			name: "Sean Banan",
			email: "sean@banan.se",
			password: "$2b$10$Ar9TyvpB8lvEGblknNbaV.M8HMyB/y3v7RVrdksTMXuQutzP4cGR2",  // abc123
		},
	});
}
